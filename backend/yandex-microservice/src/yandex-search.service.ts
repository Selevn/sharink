...
import { Link, ScrapEntity } from "sharink-lib"
...
export class YandexMusicScrapper {
  ...
  async getTrack(entity: ScrapEntity): Promise<Link> {
    const result = (await this.api.searchTracks(
      `${entity.name} ${entity.author}`
    ))!
    console.log(`results`, result.tracks.results)
    return `https://music.yandex.ru/album/${
      result.tracks.results[0]!.albums[0]!.id
    }/track/${result.tracks.results[0]!.id}`
  }
}
