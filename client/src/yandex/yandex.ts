...
checkPlayerReadiness(initialize2);
...
function initialize2() {
  ...
  shareButton.classList.add('sharink-yandex-button');

  const shareButtonImage = document.createElement('img');
  shareButtonImage.classList.add('sharink-yandex-button-image');
  shareButtonImage.src = chrome.runtime.getURL('icons/share.png');
  ...
  (async () => {
    try {
      chrome.runtime.sendMessage({ action: 'sendRequest', data: { name, author, cover } }, async response => {
        ...
        await navigator.clipboard.writeText(`http://localhost:8080/track/${response.data.responseObject.id}/page`);
